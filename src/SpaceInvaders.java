...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener, Runnable {
    ...
    private boolean hasLostGame() {
        ...
        for(int i = 0; i < alienBullets.size();i++) {
            for(int j = 0; j < aliens.length; j++){
                if((alienBullets.get(i).x >= (player.x - 15) && alienBullets.get(i).x <= (player.x + 15)) && player.y <= (alienBullets.get(i).y + 15)&&
                        player.y >= (alienBullets.get(i).y - 15)){
                    lose = true;
                }else if(aliens[j].visible == true){
                    if(aliens[j].x >= (player.x - 15) && aliens[j].x <= (player.x + 15) && aliens[j].y >= (player.y + 15) && aliens[j].y <= (player.y - 15)){
                        lose = true;
                    }
                }
            }
